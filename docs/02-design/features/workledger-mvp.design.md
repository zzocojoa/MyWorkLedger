...
- 메모는 기록 수정 화면에서 보정한다.
- 하루 전체를 `야근`으로 분류하는 태그는 연장/야간근로 계산 근거로 사용하지 않는다.
- 홈 화면은 `docs/02-design/mockup.md`의 출근 전, 근무 중, 퇴근 후 3개 상태를 따른다.

### Optional Work Rule Setup

근무 기준 설정은 연장/야간 후보 계산에 필요하지만 첫 실행 필수 등록으로 두지 않는다. 앱의 핵심 가치는 10초 출퇴근 기록이므로, 사용자는 설정 없이도 바로 출근/퇴근 기록을 시작할 수 있어야 한다.
...
| 평일 근무 요일 | 평일 기준 후보 계산 대상 요일 |

Rules:

- 기본 추천값을 제공할 수 있지만 저장은 사용자가 명시적으로 선택해야 한다.
- 설정이 없으면 연장/야간 후보를 계산하지 않는다.
- 이 설정은 회사 근태 규칙, 급여 계산, 법정 수당 정확성을 보장하지 않는다.

### Overtime And Night Candidate Rules

연장/야간근로는 수동 태그가 아니라 근무 기준과 실제 출퇴근 시각으로 계산되는 후보 구간이다.

| Candidate | Rule |
|---|---|
| 연장 후보 | 평일 근무 기준의 정시 퇴근 이후 실제 근무 구간 |
| 야간 후보 | 실제 근무 구간 중 22:00-06:00과 겹치는 구간 |
| 기록 지연 | 실제 추가 근무가 아니라 퇴근 버튼을 늦게 누른 사유 |

Rules:

- 연장 후보와 야간 후보는 겹칠 수 있으므로 하나의 태그로 합치지 않는다.
- 사용자가 `기록 지연`을 선택하면 해당 늦은 퇴근은 연장/야간 후보 확정값처럼 표시하지 않는다.
- 월간 요약에서는 후보 시간과 확정 여부를 구분해 표시한다.
- MVP 현재 구현의 `WorkRecordTag`는 개인 참고 태그로만 유지하고, 향후 후보 구간 모델로 대체한다.
...
| 월간 연장/야간 후보 | 근무 기준 설정이 있을 때 후보 구간 합계와 확인 필요 상태를 별도 표시 |
| 연차 사용량 | 선택 월 `LeaveUsage.usedLeaveMinutes` 합계 |
| 잔여연차 | 연도 총량에서 연도 사용량 합계 차감 |

월간 연장/야간 후보는 임금 산정값이 아니라 개인 참고용으로 표시한다. 근무 기준이 설정되지 않은 경우에는 총 근무시간만 표시하고 후보 시간을 계산하지 않는다.
...
| 월간 요약 | 월간 근무/선택형 연장·야간 후보/연차 요약 |
...
| Monthly summary | 근무시간 합계, 근무 기준 미설정 시 후보 미표시, 근무 기준 설정 시 연장/야간 후보 구간 합계 |
