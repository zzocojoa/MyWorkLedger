...
이번 단계에서는 코드, 이미지, Android 리소스를 만들지 않는다. 디자이너와 개발자가 같은 기준으로 다음 작업을 시작할 수 있게 문서만 준비한다.
...
- 배경은 `DESIGN-airtable.md`의 `surface-dark` 계열인 `#181d26` 또는 signature 계열 중 하나를 사용한다.
- 중앙 심볼은 흰색 또는 밝은 surface 색으로 둔다.
...
권장 확인 명령은 후속 Android 리소스 반영 단계에서 실행한다:
...
1. 로고 시안 2-3개를 만든다.
2. 각 시안을 `512px`, `96px`, `72px`, `48px`로 축소해 읽힘을 비교한다.
3. 선택한 시안으로 Play app icon, adaptive icon foreground/background, monochrome layer를 분리한다.
4. feature graphic은 앱 아이콘 확대판이 아니라 실제 앱 가치가 보이는 store preview로 만든다.
5. Android 리소스 반영 단계에서 실제 기기 홈 화면과 Play Console preview를 확인한다.

이번 문서 작업에서는 이미지 파일과 Android 리소스를 만들지 않는다. 다음 단계에서 source asset이 확정되면 이 문서를 기준으로 export와 앱 리소스 반영을 진행한다.
