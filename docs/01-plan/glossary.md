...
| Work Rule | 근무 기준 | 정시 출근, 정시 퇴근, 연장 근무 태그 시작, 야간 근무 시작, 휴게시간, 근무 요일 설정 | 선택 설정이며 앱 첫 사용을 막지 않는다. 월간 요약은 저장된 근무 기준이 없으면 기본 참고 기준을 사용한다 |
| Work Tag | 근무 태그 | 근무 기준과 출퇴근 시각으로 계산한 근무 구간 분류 | 휴무일 근무, 정시 전 근무, 연장 근무, 야간 근무를 각각 분리해 표시한다. 저장된 근무 기준이 없어도 월간 요약에서는 기본 참고 기준으로 보여준다 |
...
| Overtime Work | 연장 근무 | 연장 근무 태그 시작 시간 이후 실제 근무 구간 | 기본값은 정시 퇴근이며 야간 근무와 겹칠 수 있으므로 단순 합산하지 않는다 |
