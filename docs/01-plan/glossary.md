...
| Work Rule | 근무 기준 | 정시 출근, 정시 퇴근, 연장 근무 시작 시간, 야간 근무 시작 시간, 휴게시간, 근무 요일 설정 | 선택 설정이며 앱 첫 사용을 막지 않는다. 월간 요약은 저장된 근무 기준이 없으면 기본 참고 기준을 사용한다 |
| Work Tag | 근무 태그 | 근무 기준과 출퇴근 시각으로 계산한 근무 구간 분류 | 휴무일 근무, 정시 근무, 정시 전 근무, 연장 근무, 야간 근무를 각각 분리해 표시한다. 저장된 근무 기준이 없어도 월간 요약과 달력 보기에서는 기본 참고 기준으로 보여준다 |
| Non-Workday Work | 휴무일 근무 | 근무 요일에 포함되지 않은 날짜의 휴게시간 제외 근무 구간 | 정시 전/연장/야간 근무와 동시에 표시될 수 있다 |
| Regular Work | 정시 근무 | 근무 요일의 정시 출근부터 정시 퇴근까지 실제 근무와 겹친 구간에서 휴게시간을 제외한 시간 | 휴무일 기록에는 중복 표시하지 않는다 |
| Early Work | 정시 전 근무 | 정시 출근 이전 실제 근무 구간 | 임금 산정이나 법적 판단을 의미하지 않는다 |
| Overtime Work | 연장 근무 | 연장 근무 시작 시간 이후 실제 근무 구간 | 기본값은 정시 퇴근이며 야간 근무와 겹칠 수 있으므로 단순 합산하지 않는다 |
| Night Work | 야간 근무 | 야간 근무 시작 시간부터 8시간 구간과 겹치는 실제 근무 구간 | 기본값은 22:00이며 개인 참고용이고 임금 정확성을 보장하지 않는다 |
...
| 기록 사유 | `WorkRecordTag` | serialized list | 기존 저장 구조와 호환하되 월간 근무 태그 계산 근거로 쓰지 않는다. 값이 있으면 월간 요약의 `태그별 참고`에 별도로 표시한다 |
