...
근무 태그 계산을 위한 선택 설정이다. 설정하지 않아도 출퇴근 기록은 사용할 수 있다. 월간 요약은 저장된 `WorkRule`이 없을 때도 기본 참고 기준(09:00-18:00, 휴게 60분, 월-금, 야간 22:00)으로 근무 태그를 계산해 보여준다. 이 기본 참고 기준은 화면 계산용이며 `WorkRule`로 자동 저장하지 않는다.
...
- `memo`는 비어 있거나 500자 이하 문자열이어야 한다.
- 기준 연도의 `LeaveBalance`가 없으면 연차 사용 추가를 막고 총 연차 등록을 안내한다.
- 연차 관리 리셋은 기준 연도의 `LeaveUsage` 목록을 삭제해 사용 일수와 시간을 초기화한다.
