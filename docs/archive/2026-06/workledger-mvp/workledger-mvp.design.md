...
고정 포함 시간 비교는 계약 형태를 판단하는 기능이 아니라, 사용자가 앞으로 참고할 비교 방식을 직접 고르는 기능이다. `고정 포함 시간 있음`일 때 월간 요약에 `포함 시간 대비` 카드를 표시한다.
...
| 포함 시간 | `fixedIncludedAfterRegularEndMinutes` |
...
월간 요약 로드
  ...
  -> 고정 포함 시간 있음이면 정시 퇴근 이후 근무를 입력한 고정 포함 시간과 비교
  -> 고정 포함 시간 없음 또는 잘 모르겠음이면 비교 카드 숨김
```

야간 근무와 휴무일 근무는 포함 시간 비교 입력에서 제외한다. 해당 시간은 기존 월간 요약의 근무 태그로 별도 표시하고, 비교 카드는 정시 퇴근 이후 실제 근무 중 얼마가 사용자가 입력한 고정 포함 시간 안에 들어가는지만 보여준다.
...
- 설정 홈에서는 `근무 기준`과 `비교 방식`을 별도 항목으로 나누지 않고 `근무 설정` 한 화면에서 입력한다.
- `근무 설정` 화면은 `기본 근무 기준`, `추가 근무 기준`, `포함 시간 비교` 섹션으로 나눈다.
- 화면은 통합하지만 `WorkRule`과 `CompensationReferenceSetting` 저장 모델은 분리 유지한다.
...
- `포함 시간 대비` 카드는 `정시 이후 근무`의 `실제 기록`, `포함 시간`, `초과 참고`만 표시한다.
...
| 기록 추가/수정 | 선택한 근무일의 출근/퇴근 시각, 기록 사유, 메모 보정 |
| 달력 보기 | 이번 달 날짜별 근무 기록 유무와 완료/미완료 상태 확인, 누락 기록 추가/수정 |
...
앱 실행
  ...
  -> 선택 날짜에 기록이 없으면 기록 추가
  -> 선택 날짜에 기존 기록이 있으면 기록 수정
  -> 월간 요약
  -> 월간 리포트 만들기
  -> 가격표/fake-door
```
