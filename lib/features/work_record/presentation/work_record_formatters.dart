...
String formatEditableWorkRecordReason({required WorkRecordTag tag}) {
  return switch (tag) {
...
String formatEditableWorkRecordReasons({required List<WorkRecordTag> tags}) {
  return tags
      .where(editableWorkRecordReasonTags.contains)
      .map((WorkRecordTag tag) => formatEditableWorkRecordReason(tag: tag))
      .join(' · ');
}
